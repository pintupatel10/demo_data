...
class ReportController extends Controller
{
        ...
        if(!empty($request['submit'])) {
            ...
            if ($request['report_for'] == 'Student') {
                ...
                if ($request['report_type'] == 'Attendance Today' || $request['report_type'] == 'Attendance By Date') {
                    ...
                    $data00['type'] = 'Student';
                    $data00['report'] = $request['report_type'];
                    $data00['data'] = $student;
                   // session(['data' => $data]);
                }
            ...
            if ($request['report_for'] == 'Staff') {
                ...
                if ($request['report_type_staff'] == 'Attendance Today' || $request['report_type_staff'] == 'Attendance By Date') {
                    ...
                    $data00['type'] = 'Staff';
                    $data00['report'] = $request['report_type_staff'];
                    $data00['data'] = $staff;
                   // session(['data' => $data]);
                }
            ...
            File::put($destinationPath.$file,$d);
        }
    ...
    public function export(Request $request)
    {
        ...
        if (!is_dir($destinationPath)) {  mkdir($destinationPath,0777,true);  }
        if(file_exists($destinationPath.$file)) {
           // $get_file_data=File::get($destinationPath.$file);
            ...
            Excel::create('type' . '-' . 'report', function ($excel) use ($json) {
                $excel->sheet('report', function ($sheet) use ($json) {
                    $sheet->fromArray($json['data']);
                });
        ...
        if(!empty($request['export'])) {
            ...
            if ($request['report_for'] == 'Student') {
                ...
                if ($request['report_type'] == 'Attendance Today' || $request['report_type'] == 'Attendance By Date') {
                    ...
                    Excel::create($data00['type'] . '-' . $data00['report'], function ($excel) use ($data) {
                        $excel->sheet($data00['report'], function ($sheet) use ($data) {
                            $sheet->fromArray($data00['data']);
                        });

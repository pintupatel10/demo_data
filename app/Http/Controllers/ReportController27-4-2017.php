...
class ReportController extends Controller
{
        ...
        if($request['report_for'] == 'Student') {
            ...
            if ($request['report_type'] == 'Attendance By Device') {
                ...
                foreach ($Dates as $key => $date) {
                    $i = strtotime($date);
                    if (date('N', $i) <= 6 && !in_array(date('Y-m-d', $i), $holiday)) {
                        $check_present = Attendance::where('student_id', $request['student_id'])->where('device_id',$request['device'])->where('attendance_date', $date)->first();
                        if (!empty($check_present)) {
                            if ($check_present->on_leave == 1) {
                                $data[$key]['Date'] = $date;
                                $data[$key]['Attendance'] = 'Leave';
                ...
                Excel::create('Student ' . $request['report_type'], function ($excel) use ($data,$request) {

                    $excel->sheet('Attendance'.$request['from'].'-'.$request['to'], function ($sheet) use ($data) {
                        $sheet->fromArray($data);
                    });
        ...
        if($request['report_for']=='Staff'){
            ...
            if ($request['report_type_staff'] == 'Attendance By Device') {
                ...
                foreach ($Dates as $key => $date) {
                    $i = strtotime($date);
                    if (date('N', $i) <= 6 && !in_array(date('Y-m-d', $i), $holiday)) {
                        $check_present = Attendance::where('staff_id', $request['staff_id'])->where('device_id',$request['device'])->where('attendance_date', $date)->first();
                        if (!empty($check_present)) {
                            if ($check_present->on_leave == 1) {
                                $data[$key]['Date'] = $date;
                                $data[$key]['Attendance'] = 'Leave';
                ...
                Excel::create('Staff '.$request['report_type_staff'], function ($excel) use ($data,$request) {

                    $excel->sheet('Attendance'.$request['staff_date_from'].'-'.$request['staff_date_to'], function ($sheet) use ($data) {
                        $sheet->fromArray($data);
                    });
    ...
    public function get_device($school_id)
    {
        ...
        foreach ($device as $device1){
            echo '<option value="'.$device1['id'].'">'.$device1['name'].'</option>';
        }
        return "";
    }

...
class DivisionController extends Controller
{
    ...
    public function index()
    {
      ...
      //  $data['division'] = Division::with('school')->with('classm')->get();
        ...
        if(Auth::user()->role == "staff") {
            $data['division'] = Division::with('school')->with('classm')->where('school_id', Auth::user()->school_id)->get();
        } else {
            $data['division'] = Division::with('school')->with('classm')->get();
        }
    ...
    public function create()
    {
        ...
        if(Auth::user()->role == "admin") {
            ...
            $data['name1']=Class_Master::where('school_id', Auth::user()->school_id)->lists('name','id')->all();
        }
    ...
    public function edit(Request $request,$id)
    {
        ...
        $data['division'] = Division::findorFail($id);
        $data['modes_selected'] = $data['division']['school_id'];
        $data['modes'] = $data['division']['class_id'];
        $data['modes1'] = $data['division']['division'];

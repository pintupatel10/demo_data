        ...
        if(!empty($request['submit'])) {
            ...
            if ($request['report_for'] == 'Student') {
                ...
                if ($request['report_type'] == 'Attendance By Month') {
                    ...
                    foreach ($student as $key => $value) {
                        ...
                        for ($i = $start_time; $i < $end_time; $i += 86400) {
                            $future_date=Carbon::parse(date('Y-m-d', $i));
                            if($future_date->isFuture()){
                                break;
                            }
                            if (!in_array(date('Y-m-d', $i), $holiday)) {
                                $working_days++;
                ...
                if ($request['report_type'] == 'Attendance By Year') {
                    ...
                    foreach ($student as $key => $value) {
                        ...
                        foreach ($months as $key1 => $value1) {
                            ...
                            $end_time = strtotime("+1 month", $start_time);
                            for ($i = $start_time; $i < $end_time; $i += 86400) {
                                $date_in_future=Carbon::parse(date('Y-m-d', $i));

                                if($date_in_future->isFuture()){
                                    $break=1;
                                }
                                else {
                                    if (!in_array(date('Y-m-d', $i), $holiday)) {
                ...
                if ($request['report_type'] == 'Attendance By Device') {
                    ...
                    $date_facturation_from=Carbon::parse($request['from']);
                    if($date_facturation_from->isFuture()){
                        return back()->withInput()->withErrors(['from' => 'Date is in future please select other Date !']);
                    }
                    $date_facturation_to=Carbon::parse($request['to']);

                    if($date_facturation_to->isFuture()){
                        return back()->withInput()->withErrors(['to' => 'Date is in future please select other Date !']);
                    }
           ...
           // return $data['data'];
            if ($request['report_for'] == 'Staff') {
                ...
                if ($request['report_type_staff'] == 'Attendance By Device') {
                    ...
                    $date_facturation_from=Carbon::parse($request['staff_date_from']);

                    if($date_facturation_from->isFuture()){
                        return back()->withInput()->withErrors(['staff_date_from' => 'Date is in future please select other Date !']);
                    }

                    $date_facturation_to=Carbon::parse($request['staff_date_to']);

                    if($date_facturation_to->isFuture()){
                        return back()->withInput()->withErrors(['staff_date_to' => 'Date is in future please select other Date !']);
                    }
    ...
    public function export(Request $request)
    {
        if(!empty($request['export'])) {
            ...
            if ($request['report_for'] == 'Student') {
                ...
                $date = Carbon::today()->toDateString();
                /*for year and month report load only student id,name,rollno, class*/
                if ($request['report_type'] == 'Attendance By Year' || $request['report_type'] == 'Attendance By Month') {
                    if ($request['student_id'] != '') {
                        $student = Student::select('roll_no','name','school_id','class_id')
                            ->where('role', 'student')->where('id', $request['student_id'])->get();
                    } else if ($request['school_id'] != '' && $request['class_name'] != '' && $request['class_division'] != '') {
                        $student = Student::select('roll_no','name','school_id','class_id')
                            ->where('role', 'student')->where('school_id', $request['school_id'])->where('class_id', $request['class_name'])->where('division', $request['class_division'])->get();

                    } else if ($request['school_id'] != '' && $request['class_name'] != '') {
                        $student = Student::select('roll_no','name','school_id','class_id')
                            ->where('role', 'student')->where('school_id', $request['school_id'])->where('class_id', $request['class_name'])->get();

                    } else if ($request['school_id'] != '') {
                        $student = Student::select('roll_no','name','school_id','class_id')
                            ->where('role', 'student')->where('school_id', $request['school_id'])->get();
                    } else {
                        $student = new \Illuminate\Database\Eloquent\Collection;
                    }
                }

                /*else get all data of student*/

                else{

                    if ($request['student_id'] != '') {
                        $student = Student::select('roll_no', 'name', 'school_id', 'class_id', 'division', 'address', 'birthdate', 'blood_group', 'mobile', 'school_time', 'parents_name', 'rfid_no')
                            ->where('role', 'student')->where('id', $request['student_id'])->get();
                    } else if ($request['school_id'] != '' && $request['class_name'] != '' && $request['class_division'] != '') {
                        $student = Student::select('roll_no', 'name', 'school_id', 'class_id', 'division', 'address', 'birthdate', 'blood_group', 'mobile', 'school_time', 'parents_name', 'rfid_no')
                            ->where('role', 'student')->where('school_id', $request['school_id'])->where('class_id', $request['class_name'])->where('division', $request['class_division'])->get();
                ...
                if ($request['report_type'] == 'Attendance Today' || $request['report_type'] == 'Attendance By Date') {
                    ...
                    try {
                        Excel::create($data['type'] . '-' . $data['report'], function ($excel) use ($data, $request) {
                            $excel->sheet($data['report'], function ($sheet) use ($data, $request) {
                                if ($request['export_type'] == 'pdf') {
                                    $sheet->loadView('report.format_date_student', $data);
                ...
                if ($request['report_type'] == 'Attendance By Month') {
                    ...
                    try{
                    Excel::create($data['type'] . '-' . $data['report'], function ($excel) use ($data,$ssdata,$days,$class_name,$Displaymonth,$request) {
                        $excel->sheet($data['report'], function ($sheet) use ($ssdata,$days,$class_name,$Displaymonth,$request) {
                ...
                if ($request['report_type'] == 'Attendance By Year') {
                    ...
                    try{
                       Excel::create($data['type'] . '-' . $data['report'].' '.$request['yearfrom'].'-'.$year, function ($excel) use ($data,$sdata,$ssdisplay,$class_name,$request) {
                        $excel->sheet($data['report'], function ($sheet) use ($sdata,$class_name, $ssdisplay,$request) {
                            $i = 0;
                            $break=0;
                            $data100=array();
                ...
                if ($request['report_type'] == 'Attendance By Device') {
                    ...
                    $class_name=Class_Master::where('id',$student->class_id)->first();

                    $device=Device::where('id',$request['device'])->first();
                    $from=Carbon::parse($request['from'])->format('Y-m-d');
                    $to=Carbon::parse($request['to'])->format('Y-m-d');

                    $check_present = AttendanceDetail::select('rfid_no','student_id','class_name','class_division','school_in_time','school_out_time','attendance_date','attendance_time','device_id','latittude','longitude')->where('student_id', $request['student_id'])->where('device_id', $request['device'])->where('attendance_date','>=',$from)->where('attendance_date','<=',$to)->get();

                    foreach($check_present as $key=>$value){
                        $check_present[$key]['student_id']=$student->name;
                        $check_present[$key]['class_name']=$class_name['name'];
                        $check_present[$key]['device_id']=$device['name'];
                    }
            ...
            if ($request['report_for'] == 'Staff') {
                ...
                if ($request['staff_id'] != '') {
                    $staff = Staff::select('name','email','staff_role','school_id','address','birthdate','blood_group','mobile','school_time','rfid_no')->where('role', 'staff')->where('id', $request['staff_id'])->get();
                } else if ($request['staff_school_id'] != '') {
                    $staff = Staff::select('name','email','staff_role','school_id','address','birthdate','blood_group','mobile','school_time','rfid_no')->where('role', 'staff')->where('school_id', $request['staff_school_id'])->get();
                } else {
                    $staff = new \Illuminate\Database\Eloquent\Collection;
                }
                ...
                if ($request['report_type_staff'] == 'Attendance Today' || $request['report_type_staff'] == 'Attendance By Date') {
                    ...
                    $school_name = School::select('name')->where('id', $request['staff_school_id'])->first();
                    ...
                    Excel::create($data['type'] . '-' . $data['report'], function ($excel) use ($data,$request) {
                        $excel->sheet($data['report'], function ($sheet) use ($data,$request) {
                            if ($request['export_type'] == 'pdf') {
                                $sheet->loadView('report.format_date_staff', $data);
                                $sheet->setOrientation('landscape');
                                //  $sheet->setAutoSize(true);
                            }
                            $sheet->fromArray($data['data']);
                ...
                if ($request['report_type_staff'] == 'Attendance By Month') {
                  ...
                  //  $ssdata=array_values($sdata);
                    ...
                    Excel::create($data['type'] . '-' . $data['report'].' '.$Displaymonth, function ($excel) use ($data,$sdata,$days,$school_name,$Displaymonth,$request) {
                        $excel->sheet($data['report'], function ($sheet) use ($sdata,$days,$school_name,$Displaymonth,$request) {
                ...
                if ($request['report_type_staff'] == 'Attendance By Year') {
                    ...
                    Excel::create($data['type'] . '-' . $data['report'].' '.$request['yearfrom_staff'].'-'.$year, function ($excel) use ($data,$sdata,$ssdisplay,$request) {
                        $excel->sheet($data['report'], function ($sheet) use ($sdata, $ssdisplay,$request) {
